 ...
 * 7.统一设置ViewPage是否需要滑动返回如只有在第一屏的时候需要滑动返回设置是否需要滑动返回setIsNeedClose
 *   【如果子类有需要在某种情况下不需要返回直接通过mSwipeBackLayout.setEdgeSize(0)即可
 *
 * ============================
 *
...
public abstract class MySwipeBackActivity extends SwipeBackActivity implements View.OnClickListener {
    ...
    public void addRefresh(PtrFrameLayout ptrMain, final ListView lvContent) {
        ...
        if (lvContent instanceof PullToLoadMoreListView) {
            ((PullToLoadMoreListView) lvContent).setOnLoadMoreListener(new PullToLoadMoreListView.OnLoadMoreListener() {
                @Override
...
//        this.overridePendingTransition(R.anim.activity_right_in, 0);
    ...
    /**
     * 设置是否需要滑动返回:当ViewPage做布局文件时有时在第二页 或者第三页滑动会导致侧滑返回被触发
     * @param vpContent ViewPage控件
     */
    public void setIsNeedClose(ViewPager vpContent) {
